...
## FEAT: Pinch/scroll-to-zoom and pan in the Memory Gallery lightbox (2026-07-04)
- **Why**: Some captured screenshots contain small text (code, terminal output) that's unreadable at the lightbox's fit-to-screen size. User asked to be able to zoom into an opened image.
- **What Changed**:
  - `public/app.js`: Added zoom/pan state (`zoomScale`, `panX`, `panY`) scoped to the lightbox, reset on every `renderLightbox()` call so each asset opens at 1x. `zoomTo()`/`toggleZoomAt()` anchor the zoom at a screen point (cursor or tap) so that point stays fixed instead of the image jumping — desktop wheel/trackpad zooms toward the cursor, double-click toggles 1x↔2.5x toward the click point. Mouse drag pans while zoomed (`mousedown`/`mousemove`/`mouseup` on window so the drag doesn't break if the cursor leaves the image). Touch handling was rewritten into a single gesture-mode state machine (`'pinch' | 'pan' | 'swipe'`) so two fingers pinch-zoom, one finger pans while already zoomed, one finger swipes prev/next at 1x (unchanged behavior), and a manual double-tap detector (two taps <300ms apart, <40px apart) toggles zoom the same way double-click does. `clampPan()` keeps the pan offset within the scaled image bounds so you can't drag it fully off-screen. Arrow-key prev/next navigation is suppressed while zoomed (so ArrowLeft/Right don't fight with a zoomed image) and re-enabled automatically once zoom resets to 1x.
  - `public/index.html`: Added `#lightbox-hint` ("pinch or scroll to zoom · double-tap to reset"), shown only for image assets and fading out after a few seconds; bumped `app.js`/CSS cache-bust to `v10`.
  - `public/style.css`: `.lightbox-stage img` gets `cursor: zoom-in`, a short transform transition for discrete zoom steps (disabled during active drag/pinch for lag-free continuous panning), and `touch-action: none` so our own gesture handlers fully own touch input on the image. `.zoomed`/`.dragging` classes swap the cursor to `grab`/`grabbing`.
- **Verified**: Rebuilt and redeployed the container; headless-Chromium session opened a real captured screenshot in the lightbox and confirmed: wheel-zoom anchors correctly (`translate(0px, 0.2px) scale(1.2)`, i.e. barely shifts since cursor was centered), drag-to-pan moves the image (`translate(-44px, -29px)`), double-click resets to `scale(1)` then a second double-click zooms back in toward the new click point (`scale(2.5)` with a different pan offset, confirming point-anchored zoom rather than always-center), ArrowRight is a no-op while zoomed and advances the gallery normally once reset. Zero console errors.
